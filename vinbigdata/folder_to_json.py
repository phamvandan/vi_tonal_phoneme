...
# folder of data contain txt transcript files
folder_path = sys.argv[1]
# folder to save json file
save_dir = sys.argv[2]

save_name = os.path.join(save_dir, "vin_big_data.lst")
...
for (dirpath, dirnames, filenames) in walk(folder_path):
    for filename in filenames:
        if ".txt" in filename:
            ...
            data["duration"] = get_duration_of_audio_file(audio_path)
            data["text"] = covert_unicode(trans)
            json.dump(data, f_lst)
            f_lst.write("\n")
            index += 1
